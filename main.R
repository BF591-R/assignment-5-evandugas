...
make_se <- function(counts_csv, metafile_csv, selected_times) {
    # Read counts matrix (tab-separated, gene column as row names)
    counts <- read.delim(counts_csv, row.names = 1)

    # Read metadata
    meta <- read.csv(metafile_csv)

    # Subset metadata to selected timepoints and keep relevant columns
    coldata <- meta %>%
    ...
    # Subset counts matrix to matching samples and convert to matrix
    counts_matrix <- as.matrix(counts[, coldata$samplename])

    # Set factor levels with first element of selected_times as reference
    coldata$timepoint <- factor(coldata$timepoint, levels = selected_times)
...
return_deseq_res <- function(se, design) {
    ...
    return(list(res_df = res_df, dds = dds))
}
...
label_res <- function(deseq2_res, padj_threshold) {
    res_tib <- as_tibble(deseq2_res, rownames = 'genes')
    res_tib <- res_tib %>%
...
plot_log2fc <- function(labeled_results, padj_threshold) {
    sig_results <- labeled_results %>% filter(padj < padj_threshold)
    p <- ggplot(sig_results, aes(x = log2FoldChange)) +
        geom_histogram(bins = 30) +
        labs(x = 'Log2 Fold Change', y = 'Count',
             title = 'Log2 Fold Change of DE genes') +
        theme_minimal()
    return(p)
}
...
scatter_norm_counts <- function(labeled_results, dds_obj, num_genes){
    ...
    counts_long <- as_tibble(norm_counts[top_genes, , drop = FALSE], rownames = 'genes') %>%
        pivot_longer(cols = -genes, names_to = 'sample', values_to = 'normalized_counts')

    p <- ggplot(counts_long, aes(x = genes, y = log10(normalized_counts + 1), color = sample)) +
        geom_jitter(width = 0.2, size = 2) +
        labs(x = 'Gene', y = 'Log10(Normalized Counts + 1)',
...
make_ranked_log2fc <- function(labeled_results, id2gene_path) {
    id2gene <- read.delim(id2gene_path, header = FALSE,
                          col.names = c('genes', 'symbol'))

    results_with_symbols <- labeled_results %>%
        inner_join(id2gene, by = 'genes') %>%
        filter(!is.na(log2FoldChange), !is.na(symbol), symbol != '') %>%
        group_by(symbol) %>%
        slice_max(abs(log2FoldChange), n = 1, with_ties = FALSE) %>%
        ungroup() %>%
        arrange(desc(log2FoldChange))

    rnk <- deframe(results_with_symbols %>% dplyr::select(symbol, log2FoldChange))
    return(rnk)
}
...
top_pathways <- function(fgsea_results, num_paths){
  ...
  top_both <- top_both %>%
    mutate(pathway = str_trunc(pathway, width = 50))
  
  p <- ggplot(top_both, aes(x = reorder(pathway, NES), y = NES, fill = NES > 0)) +
    geom_col() +
    coord_flip() +
    labs(x = 'Pathway', y = 'Normalized Enrichment Score',
         title = 'Top pathways by NES', fill = 'Positive NES') +
    theme_minimal() +
